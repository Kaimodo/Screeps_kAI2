...
export function run(room: Room, creep: Creep, rm: M.RoomMemory): void {
  ...
  if (cm.assignedMineTaskId === undefined) {
    console.log(`${M.l(cm)}has no mining task`);
    const unassignedTasks = _.filter(rm.minerTasks!, (t: M.MinerTask) => t.assignedMinerName === undefined);
    console.log(`unassignedTask.length: ${unassignedTasks.length}`);
    if (unassignedTasks.length === 0) {
      console.log(`${M.l(cm)}No unassigned miner tasks found`);
    } else {
      unassignedTasks[0].assignedMinerName = creep.name;
      cm.assignedMineTaskId = unassignedTasks[0].taskId;
      console.log(`${M.l(cm)}Now assigned miner task ${cm.assignedMineTaskId}`);
    }
  } else {
...
function harvestEnergy(creep: Creep, cm: M.CreepMemory, rm: M.RoomMemory, minerTask: M.MinerTask): void {
  // log.info(`${M.l(cm)}is moving to mine`);
  if (creep.pos.x !== minerTask.minerPosition.x || creep.pos.y !== minerTask.minerPosition.y) {
    ...
    if (pos !== null) {
      ...
      console.log(`${M.l(cm)}Can't find ${pos}`);
    }
  } else {
    ...
    if (errCode !== OK && errCode !== ERR_NOT_IN_RANGE && errCode !== ERR_NOT_ENOUGH_RESOURCES) {
      console.log(`${M.l(cm)}Harvest error ${errCode}`);
    }
  }
}

function buildIfCan(room: Room, creep: Creep, rm: M.RoomMemory, cm: M.CreepMemory): boolean {
  console.log(`${M.l(cm)}buildIfCan ${room.name}, ${creep.name}`);
  ...
  if (targets.length > 0) {
    ...
    if (status === ERR_NOT_IN_RANGE) {
      ...
      if (moveCode !== OK && moveCode !== ERR_TIRED) {
        console.log(`${M.l(cm)}move and got ${moveCode}`);
      }
    }
    return true;
  } else {
    ...
    if (RoomManager.containers.length !== rm.containerPositions!.length) {
      console.log(
        `${M.l(cm)}RoomManager.containers.length=${RoomManager.containers.length}. rm.containerPositions.length=${
          rm.containerPositions!.length
        }`
      );
      _.each(rm.containerPositions!, (containerPos: M.PositionPlusTarget) => {
        console.log(`${M.l(cm)}Creating container at ${containerPos.x}, ${containerPos.y}`);
...
function dropOffEnergy(room: Room, creep: Creep, rm: M.RoomMemory, minerTask: M.MinerTask, cm: M.CreepMemory): void {
  ...
  if (minerTask.sourceContainer === undefined || rm.techLevel! < 3 || RoomManager.builders.length === 0) {
    // log.info(`${M.l(cm)}no source container or low tech`);
    if (RoomManager.containers.length === rm.containerPositions!.length && rm.techLevel! >= 3) {
      console.log(`${M.l(cm)}room has containers and tech 3+`);
      const foundContainerPos = _.find(
        rm.containerPositions!,
        (containerPos: M.PositionPlusTarget) => containerPos.targetId === minerTask.minerPosition.targetId
      );
      if (foundContainerPos !== null) {
        ...
        if (roomPos !== null) {
          ...
          if (targets.length > 0) {
            target = targets[0];
            console.log(`${M.l(cm)}Found matching containerPos ${target.id}`);
