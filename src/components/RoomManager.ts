...
export function run(room: Room, rm: M.RoomMemory): void {
  ...
  if (rm.spawnText !== undefined && rm.spawnTextId !== undefined) {
    ...
    room.visual.text(rm.spawnText, spawn.pos.x + 1, spawn.pos.y, { align: "left", opacity: 0.8 });
...
function getTechLevel(room: Room, rm: M.RoomMemory, numExtensionToBuild: number): number {
  ...
  // Tech level 5 = ?
  if (rm.minerTasks?.length !== undefined) {
    if (miners.length < rm.minerTasks.length - 2) {
      return 1;
    }
  }

  if (rm.energySources?.length !== undefined) {
    if (containers.length !== rm.energySources.length) {
      return 2;
    }
  }

  if (rm.desiredBuilders !== undefined) {
...
function scanRoom(room: Room, rm: M.RoomMemory) {
  ...
  miners = _.filter(creeps, (creep: Creep) => M.cm(creep).role === M.CreepRoles.ROLE_MINER);
  builders = _.filter(creeps, (creep: Creep) => M.cm(creep).role === M.CreepRoles.ROLE_BUILDER);
  ...
  notRoadNeedingRepair = _.filter(structures, (structure: Structure) => {
  ...
  if (room.controller != null) {
    switch (room.controller.level) {
      case 2:
        numTownersToBuild = 0;
        numExtensionToBuild = 5;
        break;
      case 3:
        numTownersToBuild = 1;
        numExtensionToBuild = 10;
        break;
      case 4:
        numTownersToBuild = 1;
        numExtensionToBuild = 20;
        break;
      case 5:
        numTownersToBuild = 2;
        numExtensionToBuild = 30;
        break;
      case 6:
        numTownersToBuild = 2;
        numExtensionToBuild = 40;
        break;
      case 7:
        numTownersToBuild = 3;
        numExtensionToBuild = 50;
        break;
      case 8:
        numTownersToBuild = 8;
        numExtensionToBuild = 60;
        break;
    }
  }
  ...
  console.log(
    `TL=${rm.techLevel} Mem:${M.m().memVersion}/${M.MemoryVersion} M:${miners.length}/${rm.minerTasks!.length} B:${
      builders.length
    }/${rm.desiredBuilders} S=${structures.length} Con=${containers.length}/${rm.containerPositions!.length} Ext=${
      extensions.length
    }/${numExtensionToBuild} R:${notRoadNeedingRepair.length} E:${rm.extensionIdsAssigned!.length}`
  );
}
  ...
  if (miners.length < rm.minerTasks!.length) {
    switch (rm.energyLevel) {
      case 1:
        ...
        break; // 300
      case 2:
        bodyParts = [WORK, WORK, WORK, WORK, CARRY, CARRY, MOVE];
        break; // 550
      default:
      case 3:
        bodyParts = [WORK, WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, MOVE];
        break; // 6x100,3x50=750
    }
...
function getOptimalExtensionPosition(room: Room, rm: M.RoomMemory, extPositions: RoomPosition[]): RoomPosition | null {
  ...
  console.log(`finding optimal extension pos`);
    ...
    for (let y = firstSpawn.pos.y - maxRange; y < firstSpawn.pos.y + maxRange; y++) {
      ...
      if (searchRoomPos !== null) {
        ...
        if (found != "wall") {
          ...
          // console.log(`Choice is ${x}, ${y} == ${range}`);
  ...
  if (sortedChoices.length > 0) {
    console.log(`Best choice is ${sortedChoices[0].x}, ${sortedChoices[0].y} == ${sortedChoices[0].dist}`);
...
function buildExtension(rm: M.RoomMemory, room: Room, numExtensionToBuild: number) {
  const extConstructionSites = room.find(FIND_MY_CONSTRUCTION_SITES, {
    filter: (structure: ConstructionSite) => structure.structureType === STRUCTURE_EXTENSION
  });
  ...
  if (numExtensionsNeeded > 0) {
    ...
    console.log(`numExtensionsNeeded=${numExtensionsNeeded}`);
    ...
    if (roomPos != null) {
      ...
      if (errCode === OK) {
        console.log(`Created extension at ${roomPos}`);
        return;
      } else {
        console.log(`ERROR: created extension at ${roomPos} ${errCode}`);
      }
    } else {
      console.log(`ERROR: coudln't create more extensions`);
    }
  }
}
  ...
  for (const sourceName in sources) {
    ...
    for (const pos of positions) {
      ...
      if (roomPos !== null) {
        ...
        if (found != "wall") {
          //  tslint:disable-line
          console.log("pos " + pos[0] + "," + pos[1] + "=" + found);
...
function getOptimalContainerPosition(
  minerTasksForSource: M.MinerTask[],
  sourcePos: M.PositionPlusTarget,
  room: Room
): M.PositionPlusTarget | null {
  ...
  const choices: NodeChoice[] = [];
  console.log(`finding optimal container pos for ${sourcePos.x}, ${sourcePos.y}`);
    ...
    for (let y = sourcePos.y - 2; y <= sourcePos.y + 2; y++) {
      ...
      if (range === 2) {
        ...
        if (searchPos !== null) {
          ...
          if (found != "wall") {
            //  tslint:disable-line
            // console.log(`${x}, ${y} == ${range} is not wall`);
            ...
            // console.log(`${x}, ${y} == ${dist} total`);
            dist += firstSpawn.pos.getRangeTo(x, y);
            console.log(`${x}, ${y} == ${dist} total dist including to spawn`);
  ...
  for (const task of rm.minerTasks!) {
    if (task.assignedMinerName !== undefined) {
      ...
      if ((creep as any) === undefined) {
        console.log(`Clearing mining task assigned to ${task.assignedMinerName}`);
...
export function getContainerIdWithLeastBuildersAssigned(room: Room, rm: M.RoomMemory): string | undefined {
  ...
  _.each(containers, (container: StructureContainer) => {
    ...
    console.log(`Container ${container.id} = ${count}`);
    choices.push(choice);
  });

  const sortedChoices = _.sortBy(choices, (choice: M.NodeContainerIdChoice) => choice.count);
  if (sortedChoices.length > 0) {
    console.log(`Best container ${sortedChoices[0].id} = ${sortedChoices[0].count}`);
...
export function removeAssignedExt(targetId: string, rm: M.RoomMemory) {
  // console.log(`was rm.extensionIdsAssigned = ${rm.extensionIdsAssigned.length}`);
  _.remove(rm.extensionIdsAssigned!, (ext: string) => ext === targetId);
  // console.log(`now rm.extensionIdsAssigned = ${rm.extensionIdsAssigned.length}`);
}
